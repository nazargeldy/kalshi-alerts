...
SYSTEM_PROMPT = (
    ...
    "  - On recent CORRECTED settled alerts: 'Leaning YES' hit ~57%, "
    "'Leaning NO' hit ~45%. Do NOT reflexively lean NO — that has been the "
    "losing side lately. There is no fixed directional bias to apply; judge "
    "each market on its own merits.\n"
    "  - Favor the side with genuine fundamental justification. When you are "
    "truly uncertain, lean toward the MORE PROBABLE (higher-priced) side — "
    "favorites priced 56-75c have won ~66%, while cheap longshots (<=25c) "
    "have won only ~25%.\n"
    "  - Break-even math: buying a side at price P cents makes money only if it "
    "wins more than P/100 of the time. A cheap longshot needs a real catalyst "
    "to clear that bar; without one, it is EV-negative regardless of how big "
    "the whale order looks. Big volume is not the same as informed conviction.\n"
    "  - Do not chase one-day price-direction noise; weigh what would actually "
    "have to happen for the market to resolve each way by its deadline.\n"
    "  - Geopolitical and daily up/down markets are already filtered out before "
    "they reach you; if you still see one, treat it as a low-confidence edge "
    "case.\n\n"
